...
export default function CabinetView() {
  ...
  let fallbackCabinet: any = null;
  let fallbackItems: any[] = [];

  if (dataParam) {
    try {
      const decoded = JSON.parse(decodeURIComponent(atob(dataParam)));
      fallbackCabinet = decoded?.cabinet || null;
      fallbackItems = decoded?.items || [];
    } catch (error) {
      console.error('Failed to decode cabinet QR payload', error);
    }
  }

  if (loading && !fallbackCabinet) return <div className="text-center p-8 text-slate-500">Loading Cabinet...</div>;

  const cabinet = cabinets.find(c => c.id === id) || fallbackCabinet;
  ...
  const fallbackCabinetFiles = fallbackItems.map((item: any) => ({
    ...item,
    level: Number(item.level) || 1,
    column: Number(item.column) || 1,
    isConfidential: item.isConfidential === true || String(item.isConfidential) === 'true'
  }));
  const resolvedCabinetFiles = cabinetFiles.length > 0 ? cabinetFiles : fallbackCabinetFiles;

  const cabinetQrPayload = (() => {
    try {
      const payload = {
        cabinet: {
          id: cabinet.id,
          ...
          hasLocker: cabinet.hasLocker,
          lockers: cabinet.lockers || []
        },
        items: resolvedCabinetFiles.map((item: any) => ({
          id: item.id,
          fileNumber: item.fileNumber,
          fileName: item.fileName,
          financialYear: item.financialYear || '',
          bdCollection: item.bdCollection || '',
          level: item.level,
          column: item.column,
          isLocker: item.isLocker || false,
          isConfidential: item.isConfidential === true || String(item.isConfidential) === 'true'
        }))
      };
      return encodeURIComponent(btoa(JSON.stringify(payload)));
    } catch (error) {
      console.error('Failed to build cabinet QR payload', error);
      return '';
    }
  })();

  const cabinetQrUrl = `${window.location.origin}/cabinet/${cabinet.id}${cabinetQrPayload ? `?data=${cabinetQrPayload}` : ''}`;
  ...
  const qrTargetUrl = cabinetQrUrl || currentUrl;
    ...
    <div className={`space-y-6 ${printMode ? 'print-mode' : ''}`}>
        ...
        <div className={`print-only hidden relative ${printQROnly ? 'flex-col items-center justify-center h-screen' : 'mb-8 pb-4 border-b border-gray-300'}`}>
          ...
          <div className={`flex ${printQROnly ? 'flex-col text-center' : 'items-center justify-between'} w-full`}>
            ...
            <div className={`flex flex-col items-center border-4 border-black p-4 rounded-xl bg-gray-50 ${printQROnly ? 'mt-8' : ''}`}>
              <QRCodeSVG value={qrTargetUrl} size={printQROnly ? 400 : (printLarge ? 200 : 120)} level="M" />
      ...
      {printColumnsSeparately && (
        <div className="hidden print:block w-full">
          {rows.map((row: number) => {
            const isWholeLevelLocker = cabinet.lockers?.some((l: { level: number; column?: number | null }) => l.level === row && (l.column === null || l.column === undefined));
            const customColCount = cabinet.levelColumns?.[row] || cabinet.columns;
            const specificLockerCols = cabinet.lockers?.filter((l: { level: number; column?: number | null }) => l.level === row && l.column !== null && l.column !== undefined).map((l: { level: number; column?: number | null }) => Number(l.column)) || [];
            const hasSpecificLockers = specificLockerCols.length > 0;
            const allCols = Array.from({ length: customColCount }, (_, i) => i + 1);
            const columnsToRender = hasSpecificLockers ? specificLockerCols.sort((a: number, b: number) => a - b) : allCols;
            ...
            return columnsToRender.map((col: number) => {
              const isColLocker = hasSpecificLockers || cabinet.lockers?.some((l: { level: number; column?: number | null }) => l.level === row && l.column === col);
                ...
                <div key={`row-${row}`} className="flex gap-4">
                  ...
                  <div className="flex gap-4 flex-1">
                    {isWholeLevelLocker ? (
                      ...
                      columnsToRender.map((col: number) => {
                        const isColLocker = hasSpecificLockers || cabinet.lockers?.some((l: { level: number; column?: number | null }) => l.level === row && l.column === col);
                        const filesInCell = cabinetFiles.filter(f => f.level === row && f.column === col);
