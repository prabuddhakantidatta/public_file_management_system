...
export default function FileView() {
  ...
  // Fallback to URL parameters if Firebase blocks read access or takes time
  ...
  if (!file && !fallbackName) {
    return <div className="text-center p-8 text-slate-500">Document not found.</div>;
  }

  // Create a synthetic file object from URL params if DB fetch failed/restricted
  const displayFile = file || {
    isConfidential: false,
    fileName: fallbackName,
    fileNumber: fallbackNum ? fallbackNum.split('/')[0] : '',
    financialYear: fallbackNum && fallbackNum.includes('/') ? fallbackNum.split('/')[1] : '',
    synthetic: true, // Marker to indicate data is from QR code params
    syntheticCab: fallbackCab,
    syntheticLoc: fallbackLoc
  };

  const cabinet = file ? cabinets.find(c => c.id === file.cabinetId) : null;
  const fileType = file && file.type !== 'document' ? fileTypes.find(t => t.id === file.fileTypeId) : null;
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {isSecured ? (
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-red-200 text-center space-y-6">
          ...
          <p className="text-slate-500 max-w-sm mx-auto">This document is password protected. Please enter the access code to view its details and location.</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className={`p-6 border-b flex items-start justify-between ${displayFile.isConfidential ? 'bg-amber-50 border-amber-200' : 'bg-indigo-50 border-indigo-100'}`}>
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {displayFile.isConfidential && <span className="text-xs font-bold uppercase flex items-center bg-red-100 text-red-800 px-2 py-1 rounded-md shadow-sm"><Shield className="w-3 h-3 mr-1" /> Confidential</span>}
                </div>
            ...
            <div className="bg-white p-2 rounded-xl shadow-sm border border-slate-200">
              {(() => {
                let qrVal = window.location.href;
                if (!displayFile.isConfidential && !displayFile.synthetic) {
                  ...
                  qrVal = `Name: ${displayFile.fileName || 'Document'}\nNo: ${displayFile.fileNumber}${displayFile.financialYear ? '/' + displayFile.financialYear : ''}\nCabinet: ${cabName}\nLocation: ${loc}\n\nLink: ${qrVal}`;
                }
                return <QRCodeSVG value={qrVal} size={80} level="M" />;
              })()}
            </div>
          </div>
